...
logging.basicConfig(filename='phishing_detector.log', format="[%(asctime)s] %(levelname)s %(message)s", level=logging.INFO)
...
def detect_phishings(total_number_of_daemons, daemon_number):
    ...
    with app.app_context():
        while True:
            ...
            for user in users:
                app.logger.info("Checking for user: %s" % user.register_name)
                                ...
                                if domain_info.domain_name is not None:
                                    ...
                                    if domain_creation_time > datetime.datetime.now() - datetime.timedelta(
                                        ...
                                        if is_exist_in_ppd is None:
                                            ...
                                            db.session.commit()
                                            app.logger.info("possible phishing domain added")
                                        else:
                                            continue
                        else:
                            continue
